...
**✅ ГОТОВО 09.09:** тестовая отправка успешна (HTTP 200, MessageId от Yandex). SES v2 API (SigV4, ru-central1, POST /v2/email/outbound-emails), Simple + Raw (PDF-вложения), SMTP fallback через env. DNS: SPF объединён, DKIM-селекторы Postbox подтверждены, DMARC p=none. Ключи в .env (POSTBOX_*). Рекомендация далее: поднять DMARC до p=quarantine после недели мониторинга; массовые рассылки — на этом же транспорте.
